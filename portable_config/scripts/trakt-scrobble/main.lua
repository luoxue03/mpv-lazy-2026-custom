...
local function activation()
    ...
    if status == 0 then
        ...
        send_message("Open " .. message .. "\nPress Ctrl+Alt+r when done", 60, "FF8800")
        msg.info("Open " .. message)
        mp.add_forced_key_binding("Ctrl+Alt+r", "auth-trakt", function()
            mp.remove_key_binding("auth-trakt")
            local status = auth()
            if status == 0 then
...
function start_scrobble(config, data, no_osd)
    ...
    if state and state.title then
        ...
        if (input_available or uosc_available) and not no_osd then
            mp.add_forced_key_binding("Ctrl+Alt+r", "search-trakt", function()
                message_timer:kill()
                message_overlay:remove()
            ...
            local message2 = format_message("Incorrect scrobble? Press Ctrl+Alt+r to open the search menu", "FF8800")
...
local function trackt_scrobble(force)
    ...
    elseif status == 11 then
        send_message("[trakt] Press Ctrl+Alt+r to authenticate with Trakt.tv", 10, "FF8800")
        mp.add_forced_key_binding("Ctrl+Alt+r", "auth-trakt", activation)
...
mp.register_script_message("auth-menu", function()
    activation()
end)

mp.register_script_message("complete-auth", function()
    local status = auth()
    if status == 0 then
        send_message("Trakt authentication completed.", 3, "00FF00")
        msg.info("Trakt authentication completed")
    else
        send_message("Trakt authentication failed. Request a new code if it expired.", 5, "FF0000")
        msg.error("Trakt authentication failed")
    end
end)
