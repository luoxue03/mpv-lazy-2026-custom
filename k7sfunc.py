...
def RIFE_DML(
	input : vs.VideoNode,
	model : typing.Literal[46, 422, 4221, 4251, 4151, 426, 4262] = 46,
	turbo : bool = True,
	fps_in : float = 23.976,
	fps_num : int = 2,
	fps_den : int = 1,
	...
	if not isinstance(input, vs.VideoNode) :
		raise vs.Error(f"模块 {func_name} 的子参数 input 的值无效")
	if model not in [46, 422, 4221, 4251, 4151, 426, 4262] :
		raise vs.Error(f"模块 {func_name} 的子参数 model 的值无效")
	if not isinstance(turbo, bool) :
		raise vs.Error(f"模块 {func_name} 的子参数 turbo 的值无效")
	if not isinstance(fps_in, (int, float)) or fps_in <= 0.0 :
	...
	if model in [4251, 426, 4262, 422, 4221] :  ## https://github.com/AmusementClub/vs-mlrt/blob/2adfbab790eebe51c62c886400b0662570dfe3e9/scripts/vsmlrt.py#L1031-L1032
		t_tta = False
	if t_tta :
		mdl_fname = ["rife_v4.6_ensemble", "rife_v4.15_lite_ensemble"][[46, 4151].index(model)]
	else :
		mdl_fname = ["rife_v4.6", "rife_v4.25_lite", "rife_v4.26", "rife_v4.26_heavy","rife_v4.22", "rife_v4.22_lite","rife_v4.15_lite",][[46, 4251, 426, 4262, 422, 4221, 4151].index(model)]
